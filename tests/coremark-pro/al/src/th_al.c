...
static volatile unsigned long long initial_cycles;
...
size_t al_signal_finished( void )
{
	...
	unsigned long long final_cycles = pg_perf_cycle();
	return (size_t)((final_cycles - initial_cycles) / TICKS_PER_MSEC);
#endif
}
...
size_t al_signal_now( void )
{
	...
	return elapsed;
#else
	unsigned long long now_cycles = pg_perf_cycle();
	return (size_t)((now_cycles - initial_cycles) / TICKS_PER_MSEC);
#endif
}
...
/* Use the same implementation as HOST_EXAMPLE_CODE for bare-metal */
#include "fp_shape.h"
